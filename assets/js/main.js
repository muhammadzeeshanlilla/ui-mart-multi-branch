...
const loader = () => '<span class="inline-loader" role="status" aria-label="Loading"></span>';
...
if (page==='home') {
  ...
  main.innerHTML=`<section class="hero container"><div class="hero-grid"><div class="hero-copy"><p class="eyebrow">U&I MART / ${b.cityUpper}</p><h1>${b.headline}</h1><p>${b.text}</p><div class="button-row"><a class="button" href="#branch-contact">Plan your visit ↗</a></div><div class="hero-note"><span class="dot"></span>${b.category}</div></div><div class="hero-visual"><img src="${photo(b.id)}" alt="${b.category} inspiration" fetchpriority="high"><span class="photo-caption">Illustrative image</span><div class="visual-label"><strong>U&I</strong><span>${b.city}<br>${b.label}</span></div></div></div></section><section class="container section">${heading('GET TO KNOW YOUR BRANCH',`Everyday possibilities in ${b.city}.`)}<p>${b.intro}</p><div class="filters" style="margin-top:25px">${b.tags.map(t=>`<span class="filter">${t}</span>`).join('')}</div></section>${features()}${dealsSection()}<section class="container section" id="branch-contact">${heading('COME SAY HELLO',`Visit ${b.branch_name}.`)}<div id="contacts" class="contact-grid">${loader()}</div></section>`;
} else if (isBranchPage) {
  main.replaceChildren(el('p', 'container empty-state', branchError || 'This branch is not currently available.'));
} else if (page==='deals') {
  main.innerHTML=`<section class="container page-intro"><p class="eyebrow">MORE REASONS TO VISIT</p><h1>A little extra, from U&I.</h1><p>Discover current promotions across our three branches.</p></section><section class="container section"><div class="filters" aria-label="Filter deals by branch">${['All branches',...branches.map(b=>b.city)].map((b,i)=>`<button class="filter" data-branch="${i?b:''}" aria-pressed="${!i}">${b}</button>`).join('')}</div><div id="deals-list" class="deals-grid" aria-live="polite">${loader()}</div></section>`;
} else if(page==='contact') {
  main.innerHTML=`<section class="container page-intro"><p class="eyebrow">LET’S CONNECT</p><h1>Three branches.<br>A warm welcome at each.</h1><p>Find branch contact details and opening hours below.</p></section><section class="container section"><div id="contacts" class="contact-grid">${loader()}</div></section>`;
