...
import { el, safeUrl, date } from './dom.js';
...
const branches = [
  { id: 'abu-dhabi', city: 'Abu Dhabi', category: 'Electronics & Kitchen Items', headline: 'A smarter way<br>to everyday living.', text: 'Discover electronics for a connected life, along with kitchen essentials for every day.', label: 'CONNECTED LIVING', tags: ['Electronics', 'Kitchen Items'], intro: 'From your living room to your kitchen, our Abu Dhabi branch brings technology and everyday practicality together.' },
  { id: 'dubai', city: 'Dubai', category: 'Furniture & Kitchen Items', headline: 'Make room<br>for better living.', text: 'Thoughtful furniture for your home and workspace, with kitchen essentials to bring it all together.', label: 'SPACES TO CALL YOUR OWN', tags: ['Home furniture', 'Office furniture', 'Kitchen Items'], intro: 'Our Dubai branch is dedicated to the spaces you live and work in. Explore furniture and kitchen essentials with help from our team.' },
  { id: 'sharjah', city: 'Sharjah', category: 'Hardware, Pipes & Kitchen Items', headline: 'Built for your<br>next project.', text: 'Hardware, pipes, plumbing and utility essentials — with practical solutions for your kitchen too.', label: 'PRACTICAL BY DESIGN', tags: ['Pipes & fittings', 'Hardware & plumbing', 'Utility products', 'Kitchen Items'], intro: 'Our Sharjah branch focuses on the practical side of every project, bringing hardware, plumbing, utility products and kitchen items under one roof.' },
];
...
const cards = () => `<div class="branch-grid">${branches.map((b,i)=>`<a class="branch-card" href="${href(b.id)}"><div class="branch-photo"><img src="${photo(b.id,650)}" alt="${b.category} inspiration" loading="lazy"><span class="branch-chip">0${i+1} &nbsp; ${b.city.toUpperCase()}</span></div><div class="branch-body"><h3>${b.city}</h3><p class="category">${b.category}</p><p>${b.text}</p><div class="branch-bottom">Explore ${b.city}<span aria-hidden="true">↗</span></div></div></a>`).join('')}</div>`;
...
async function loadDeals(){
  ...
  try{deals=(await api('deals')).deals;renderDeals(branches.find(b=>b.id===page)?.city||'');}
...
async function loadContacts(){
  ...
  try{
    const response=await api('branches');target.replaceChildren();
    const selected=branches.find(b=>b.id===page)?.city;
    response.branches.filter(b=>!selected||b.city===selected).forEach(b=>{
      const card=el('article','contact-card');card.append(el('p','eyebrow',b.specialization),el('h3','',b.branch_name));const dl=el('dl');
      [['Location',b.address],['Opening hours',b.opening_hours],['Phone',b.phone],['Email',b.email]].forEach(([key,value])=>{dl.append(el('dt','',key),el('dd','',value||'Details will be published soon.'));});card.append(dl);
      const actions=el('div','contact-actions');
      const links=[['Directions',safeUrl(b.map_url)],['WhatsApp',safeUrl(b.whatsapp)],['Call',/^\+?[\d ()-]{7,25}$/.test(b.phone)?`tel:${b.phone.replace(/[^+\d]/g,'')}`:''],['Email',/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(b.email)?`mailto:${b.email}`:'']];
