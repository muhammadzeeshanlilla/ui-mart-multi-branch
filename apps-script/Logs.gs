...
function dashboard_(user,payload){
  ...
  return{success:true,tab,rows:all.slice(offset,offset+limit),total:all.length,offset,limit,summary:{users:rows_('Users').length,logins:rows_('Login_Logs').filter(l=>l.status==='success').length,chats:rows_('Chat_Logs').length,deals:DataService.getDeals().filter(d=>activeBranches.some(b=>b.city===d.branch)).length}};
}
