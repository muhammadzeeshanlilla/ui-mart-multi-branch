...
function doPost(e){
  ...
  try{
    ...
    if(payload.action==='deals')return json_({success:true,deals:DataService.getDeals().filter(d=>DataService.getBranches().some(b=>b.city===d.branch))});
    ...
    const cache=CacheService.getScriptCache(),key='chat:'+payload.session_id,attempts=Number(cache.get(key)||0);
    if(attempts>=20)throw new Error('Please wait a minute before asking another question.');cache.put(key,String(attempts+1),60);
